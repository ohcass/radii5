...
import (
	...
	"github.com/ohcass/radii5/internal/downloader"
)
...
func Run(args []string) {
	...
	if opts.URL == "" {
		if opts.JSON {
			json.NewEncoder(os.Stdout).Encode(map[string]any{"event": "error", "error": "No URL provided"})
		} else {
	...
	if IsPlaylist(opts.URL) {
		if err := downloader.DownloadPlaylist(opts.URL, opts.Format, opts.OutputDir, opts.Threads, opts.Workers, opts.MediaType, opts.Quality, opts.JSON); err != nil {
			if !opts.JSON {
				color.Red("✗ %v", err)
			}
			os.Exit(1)
		...
		if err := downloader.Download(opts.URL, opts.Format, opts.OutputDir, opts.Threads, false, nil, opts.MediaType, opts.Quality, opts.JSON); err != nil {
			if !opts.JSON {
				color.Red("✗ %v", err)
			}
			os.Exit(1)
